        ...
        carrinho.forEach(item => {
            ...
            itemElement.innerHTML = `
                <div class="resumo-item-info">
                    ...
                    <div class="resumo-item-detalhes">Quantidade: ${item.quantidade}</div>
                </div>
                <div class="resumo-item-preco">R$ ${(item.precoUnitario * item.quantidade).toFixed(2)}</div>
            `;
...
function carregarDadosClienteResumo() {
    ...
    if (clienteInfo) {
        ...
        if (!clienteSalvo) {
            ...
            alert('Por favor, preencha seus dados antes de prosseguir para o pagamento.');
...
// Função para configurar métodos de pagamento
function configurarMetodosPagamento() {
    ...
    metodosRadio.forEach(radio => {
        radio.addEventListener('change', function() {
            ...
            // Atualizar classe active nos métodos
    ...
    // Ativar o método selecionado por padrão
    document.querySelector('input[name="metodo"]:checked').dispatchEvent(new Event('change'));
}
...
function processarPagamento(event) {
    ...
    if (metodoSelecionado === 'cartao-credito' || metodoSelecionado === 'cartao-debito') {
